...
> This plugin's storage API allows you to access all of this.
...
|`put_file(file_path: String, metadata := {}) -> StorageTask`| Like `put_data()`, but with a `file_path` that will have its referencing file data passed into storage.|
...
These objects are used to keep track of any of `StorageReference` function calls that involve uploading or downloading data from storage. They also contain their returning data.
| Properties | Descriptions |
|---|---|
...
## Put File
```
Firebase.Storage.ref(storage_reference).put_file(local file)
```
...
## Put String
```
Firebase.Storage.ref(storage_reference).put_string(String, {metadata})
```
...
## Get Download URL
```
Firebase.Storage.ref(storage_reference).get_download_url()
```
...
## Get Metadata
```
Firebase.Storage.ref(storage_reference).get_metadata()
```
...
## Delete Object
```
Firebase.Storage.ref(storage_reference).delete()
```
...
## List All Objects
```
Firebase.Storage.ref(storage_reference).list_all()
```
